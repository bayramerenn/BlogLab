...
export class RegisterComponent implements OnInit {
  ...
  checkPasswords (fgs: FormGroup) {
    
    const password = fgs.get('password')?.value;
    const confirmPassword = fgs.get('confirmPassword')?.value;
    
    return confirmPassword === password ? null : { isMatching: true };
  }
  onSubmit() {
